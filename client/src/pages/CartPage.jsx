...
export default function CartPage() {
  ...
  if (cart.items.length === 0) return (
    <div style={{ textAlign: 'center', padding: 60 }}>
      ...
      <button onClick={() => navigate('/')} style={{ background: '#4f46e5', color: '#fff' }}>
        Shop Now
      </button>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      ...
      {cart.items.map(item => (
        <div key={item.product} style={{
          ...
          <img src={item.image} alt={item.name}
            style={{ width: 70, height: 70, objectFit: 'cover', borderRadius: 8, background: '#f3f4f6' }} />
          <div style={{ flex: 1 }}>
          ...
          <p style={{ fontWeight: 700, minWidth: 60, textAlign: 'right' }}>
      ...
      <div style={{
        ...
        border: '1px solid #e5e5e5', marginTop: 8,
      }}>
        ...
        <button onClick={() => navigate('/checkout')}
          style={{ background: '#4f46e5', color: '#fff', width: '100%', padding: 12, fontSize: 16 }}>
          Proceed to Checkout
        </button>
      </div>
    </div>
