...
export default function CheckoutPage() {
  ...
  const placeOrder = async () => {
    ...
    if (!form.address || !form.city || !form.zip || !form.country) return toast.error('Fill all fields');
    setLoading(true);
    try {
      await api.post('/orders', { shippingAddress: form });
      toast.success('Order placed!'); navigate('/orders');
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: 24, border: '1px solid #e5e5e5' }}>
        ...
        <button onClick={placeOrder} disabled={loading} style={{ background: '#4f46e5', color: '#fff', width: '100%', padding: 12, fontSize: 16, marginTop: 8 }}>
