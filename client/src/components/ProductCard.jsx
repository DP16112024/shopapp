  ...
  return (
    <div onClick={() => navigate(`/product/${product._id}`)} style={{
      ...
      <img src={product.image} alt={product.name}
        style={{ width: '100%', height: 200, objectFit: 'cover', background: '#f3f4f6' }} />
      <div style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={addToCart} style={{ background: '#4f46e5', color: '#fff', padding: '8px 14px', fontSize: 13 }}>Add to cart</button>
        </div>
      </div>
    </div>
  );
